...
class AppRepositoryTest {
    ...
    @Before fun setUp() { userlocdao = FakeUserlocDao()
        weatherdao = FakeWeatherDao()
        repo = AppRepository(weatherdao,userlocdao) }
    @Test fun `when checkDataLoc no data Should check status false` () = runTest {
        val checkData = userlocdao.isExist()
        assertFalse(checkData) }
    @Test fun `when addDataLoc data Should exist in Userloc and status is true`() = runTest {
        val sampleDataloc = DataDummy.generateDummyUserlocEntity()
        userlocdao.addDataLoc(sampleDataloc)
        val checkData = userlocdao.isExist()
        assertTrue(checkData) }
    @Test fun `when deleteDataloc data Should empty in Userloc and status is false`() = runTest {
        val sampleDataloc = DataDummy.generateDummyUserlocEntity()
        userlocdao.addDataLoc(sampleDataloc)
        userlocdao.deleteDataLoc()
        val checkData = userlocdao.isExist()
        assertFalse(checkData) }
    @Test fun `when getDataLoc Should equal with dummy`() = runTest {
        val sampleDataloc = DataDummy.generateDummyUserlocEntity()
        userlocdao.addDataLoc(sampleDataloc)
        val sampleData = sampleDataloc.prov
        val actualData = userlocdao.getLoc().value?.prov
        assertEquals(sampleData,actualData) }
    @Test fun `when addDataWeather Should not null`() = runTest {
    ...
    @Test fun `when deleteDataWeather Should null`() = runTest {
        val sampleData = DataDummy.generateDummyWeatherEntity()
        weatherdao.addDataToLocal(sampleData)
        val dummyTime = sampleData.dateTime
    ...
    @Test fun `when readDataWeather Should equal with entity dummy`() = runTest {
